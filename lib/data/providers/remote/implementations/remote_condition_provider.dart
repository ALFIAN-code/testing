...
class RemoteConditionProvider implements IRemoteConditionProvider {

  final DioClient dioClient;

  RemoteConditionProvider(this.dioClient);

  @override
  Future<PaginationResponseModel<ConditionPaginatedModel>> getList(BaseListRequestModel model) async {
    try {
      final Response<dynamic> result = await dioClient.post(
        EndpointConstants.conditionPagination,
        data: model.toMap(),);
      final Map<String, dynamic>? data = dioClient.getResponseData(result.data);

      if (data != null) {
        return PaginationResponseModel<ConditionPaginatedModel>.fromJson(
          data,
              (Map<String, dynamic> json) {
            final int index = (data['items'] as List<dynamic>).indexOf(json) + 1;
            return ConditionPaginatedModel.fromJson(<String, dynamic>{
              ...json,
              'index': index,
            });
          },
