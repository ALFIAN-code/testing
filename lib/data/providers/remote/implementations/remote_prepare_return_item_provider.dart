...
import '../interfaces/i_remote_prepare_return_item_provider.dart';

class RemotePrepareReturnItemProvider implements IRemotePrepareReturnItemProvider {

  final DioClient _dioClient;

  RemotePrepareReturnItemProvider(this._dioClient);

  @override
  Future<PaginationResponseModel<PrepareReturnItemPaginatedModel>> getList(BaseListRequestModel request) async {
    try {
      final Response<dynamic> result = await _dioClient.post(
        EndpointConstants.prepareReturnItemPagination,
        data: request.toMap(),
      );
      ...
      if (data != null) {
        return PaginationResponseModel<
            PrepareReturnItemPaginatedModel>.fromJson(
          data,
              (Map<String, dynamic> json) =>
                  PrepareReturnItemPaginatedModel.fromJson(json),
        );
      } else {
  ...
  Future<ScanStatusCountModel> getScanStatusCount(String projectId) async {
    try {
      final Uri url = Uri.parse('${EndpointConstants.prepareReturnStatusCount}/$projectId');
  ...
  Future<void> deleteRequest(String requestId) async {
    try {
      final Response<dynamic> _ = await _dioClient.put(
        EndpointConstants.prepareReturnRemoveScan, data: <String, dynamic>{
  ...
  Future<List<ScannedItemListModel>> getScannedList(GetScannedItemListRequest request) async {
    try {
      final Response<dynamic> result = await _dioClient.post(
        EndpointConstants.prepareReturnItemScannedList, data: request.toJson(),);
      final Map<String, dynamic>? response = result.data as Map<String, dynamic>?;
  ...
  Future<void> bulkScan(List<Map<String, dynamic>> listRequest) async {
    final Response<dynamic> _ = await _dioClient.put(
      EndpointConstants.prepareReturnItemBulkScanItem, data: <String, List<Map<String, dynamic>>>{'items': listRequest},);
  }
}