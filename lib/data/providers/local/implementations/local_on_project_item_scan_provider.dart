...
class LocalOnProjectItemScanProvider implements ILocalOnProjectItemScanProvider {

  final ILocalService _localService;
  static const String _tableName = DatabaseConstants.onProjectItemScanTable;

  LocalOnProjectItemScanProvider(this._localService);

  @override
  Future<void> deleteBatch(List<String> ids) => _localService.deleteBatch(_tableName, 'id', ids);

  @override
  Future<List<OnProjectItemScanEntity>> getAll({String? projectId}) async {
    ...
    return OnProjectItemScanTable().buildModels(list);
  }

  @override
  Future<void> insert(OnProjectItemScanEntity entity) =>
      _localService.insert(_tableName, OnProjectItemScanTable().buildMap(entity));

  @override
  Future<void> truncate() => _localService.truncate(_tableName);

  @override
  Future<OnProjectItemScanEntity?> getById(String id) async {
    const String query = 'SELECT * FROM $_tableName WHERE id = ?';
    final List<Map<String, dynamic>> list = await _localService.query(query, [id]);
    return list.isNotEmpty ? OnProjectItemScanTable().buildModels(list).first : null;
  }
}