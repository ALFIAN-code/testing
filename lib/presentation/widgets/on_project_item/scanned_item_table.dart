...
class _ScannedItemTableState extends BaseUiState<ScannedItemTable> {
  ...
  Widget build(BuildContext context) => BlocListener<DetailOnProjectItemBloc, DetailOnProjectItemState>(
    listenWhen: (DetailOnProjectItemState previous, DetailOnProjectItemState current) => previous.deleteStatus != current.deleteStatus,
          ...
          builder: (BuildContext context, DetailOnProjectItemState state) {
            ...
            if (state.status.isFailure) {
              return ErrorRetryView(
                  ...
                  onRetry: () => context.read<DetailOnProjectItemBloc>().load()
              );
            }
            ...
            return GenericDataTable<ScannedItemListModel>(
              ...
              headerWidget: ActionButtonRow(
                onRefresh: () => context.read<DetailOnProjectItemBloc>().load(),
                onSearch: (String value) => context.read<DetailOnProjectItemBloc>().setSearch(value),
              ),
              columns: <TableColumn>[
                        ...
                        child: SizedBox(
                          ...
                          onPressed: () => showDialog<void>(
                              ...
                              builder: (BuildContext ctx) => ConfirmDialog(
                                ...
                                onConfirm: () {
                                  context.read<DetailOnProjectItemBloc>().deleteScan(item.itemRequestItemScanId);
